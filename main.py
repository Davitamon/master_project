...
def total_cooling(temp):
    ...
    while abs(chi_curr - chi_prev) > 1e-6:
        print('%e, %e, %e' % (T, chi_curr, chi_prev))
        beta = chi_curr * beta_m

        # print('beta: %.3f, chi: %.3f' % (beta, chi_curr))
        ...
        # print('B: %.3e G, rho: %.3e g/cm^3, f_adv: %.3f, gamma: %.3f' % (B, rho, f, gamma))
        ...
        # print('n_prot: %.3e, n_prot: %.3e, n_prot: %.3e, n_prot: %.3e, z: %.3f' % (n_prot, n_lep, n_el, n_pos, z))
        ...
        print(p_rad, p_gas, tau)
    return brem, sync, total, q_plus * (1 - f)


if __name__ == '__main__':
    temp_list = np.logspace(5, 11, 100)
    brem_list = []
    sync_list = []
    heating_list = []
    total_list = []
